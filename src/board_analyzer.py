def count_safe_moves_horizontal(board, skip_ahead=True):
    return count_moves_with_property_horizontal(board, lambda x: is_safe_move(board, x, False), skip_ahead)


def count_safe_moves_vertical(board, skip_ahead=True):
    return count_moves_with_property_vertical(board, lambda x: is_safe_move(board, x, True), skip_ahead)
...
def count_moves_horizontal(board, skip_ahead=True):
    return count_moves_with_property_vertical(board, lambda x: can_make_move(board, x, False), skip_ahead)


def count_moves_vertical(board, skip_ahead=True):
    return count_moves_with_property_vertical(board, lambda x: can_make_move(board, x, True), skip_ahead)


def count_moves_with_property_vertical(board, prop, skip_ahead=True):
    count = 0
    col = 0
...
def count_moves_with_property_horizontal(board, prop, skip_ahead=True):
    count = 0
    row = 0
    while row < board.get_height():
        col = 0
